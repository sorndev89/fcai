...
async function seed() {
    ...
    try {
        ...
        for (const pkg of pkgData) {
            try {
                await db_1.db.insert(schema_1.packages).values(pkg);
        ...
        const adminHashed = await bcryptjs_1.default.hash('admin123', 10);
        try {
        ...
        const tenantHashed = await bcryptjs_1.default.hash('tenant123', 10);
        const tenantUsers = [
        ...
        for (const t of tenantUsers) {
            try {
                await db_1.db.insert(schema_1.users).values({ ...t, password: tenantHashed });
        ...
        for (const p of pageData) {
            try {
                await db_1.db.insert(schema_1.pages).values(p);
        ...
        for (const c of customerData) {
            try {
                await db_1.db.insert(schema_1.customers).values(c);
        ...
        for (const chat of chatLogData) {
            try {
                await db_1.db.insert(schema_1.chatLogs).values({ ...chat, id: uuid() });
            }
            catch {
                // skip if exists
            }
        ...
        for (const pm of paymentData) {
            try {
                await db_1.db.insert(schema_1.payments).values({ id: uuid(), ...pm });
            }
            catch {
                // skip
            }
